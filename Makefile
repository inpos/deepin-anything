...
all:
	...
	make -C library all
	cd server && qmake -makefile -nocache QMAKE_STRIP=: PREFIX=/usr LIB_INSTALL_DIR=/usr/lib64/$(DEB_HOST_MULTIARCH) deepin-anything-server.pro && make all

install:
	mkdir -p $(DESTDIR)/usr/lib64/$(DEB_HOST_MULTIARCH)
	cp library/bin/release/* $(DESTDIR)/usr/lib64/$(DEB_HOST_MULTIARCH)
	mkdir -p $(DESTDIR)/usr/src/deepin-anything-$(VERSION)
	cp -r kernelmod/* $(DESTDIR)/usr/src/deepin-anything-$(VERSION)
	mkdir -p $(DESTDIR)/usr/lib64/modules-load.d
	echo "vfs_monitor" | tee $(DESTDIR)/usr/lib64/modules-load.d/anything.conf
